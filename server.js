...
app.get('/api/tips/public', async (req, res) => {
  ...
  try {
    const snap = await db.ref(`publicTips/${dateKey}`).once('value');
    const val  = snap.val();
...
app.delete('/api/tips/:dateKey/:tipId', async (req, res) => {
  ...
  try {
    await db.ref(`publicTips/${dateKey}/${tipId}`).remove();
...
app.post('/api/grok/predict', async (req, res) => {
  ...
  const payload = {
    ...
    messages: [
      { role: 'system', content: 'You are an expert football analyst and sports betting advisor. Analyze statistical data and provide structured match predictions. Be concise, data-driven, and direct. Format your response with clear sections using ### for headers. Use bullet points for lists.' },
  ...
  try {
    ...
    console.error('[AI] ❌', status, JSON.stringify(detail || err.message));
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Routes: GET / | POST /api/tips/post | GET /api/tips/public | DELETE /api/tips/:date/:id`);
